...
number_color = matplotlib.colors.rgb2hex(cmap(3)[:3])
number_color="black"
default_colors = [
    # r, g, b, a
    [92, 192, 98, 0.5],
...
def draw_text(fig, ax, x, y, text, color=number_color, fontsize=14, ha="center", va="center"):
    if len(text) <= 2: # this is a number
        '''
        if color == None:
            if abs(x-0.55) <=0.23 and abs(y-0.43) <= 0.23:
                color="white"
            else:
                color="black"
        '''
        #if text != '0':
        #fontsize = fontsize*(1+int(text)/50)
        fontsize *= 1.7
        #ax.axhline(xmin=x-0.01*(1+int(text)/50), xmax=x+0.01*(1+int(text)/50), y=y-0.009*(1+int(text)/50), color=color, label='_nolegend_')
    ax.text(
        x, y, text,
...
def venn5(labels, names=['A', 'B', 'C', 'D', 'E'], **options):
    ...
    ax.set_xlim(left=0.0, right=1.0)
    # body
    draw_ellipse(fig, ax, 0.428+0.07, 0.449-0.01, 0.65, 0.45, 155.0, colors[0])
    draw_ellipse(fig, ax, 0.469+0.01, 0.543-0.06, 0.65, 0.4, 82.0, colors[1])
    draw_ellipse(fig, ax, 0.558+0.02, 0.523, 0.68, 0.45, 10.0, colors[2])
    draw_ellipse(fig, ax, 0.578-0.005, 0.432+0.005, 0.67, 0.50, 122.0, colors[3])
    draw_ellipse(fig, ax, 0.489+0.02, 0.383+0.02, 0.75, 0.50, 48.0, colors[4])
    '''
    def draw_ellipse(fig, ax, x, y, w, h, a, fillcolor):
    e = patches.Ellipse(
        xy=(x, y),
    ...
    number_size = 14
    #'''r'\underline{\alpha}: '
    draw_text(fig, ax, 0.30, 0.15, labels.get('00001', ''), fontsize=number_size)
    draw_text(fig, ax, 0.72, 0.18, labels.get('00010', ''), fontsize=number_size)
    draw_text(fig, ax, 0.57, 0.17, labels.get('00011', ''), fontsize=number_size)
    draw_text(fig, ax, 0.88, 0.58, labels.get('00100', ''), fontsize=number_size)
    draw_text(fig, ax, 0.76, 0.66, labels.get('00101', ''), fontsize=number_size)
    draw_text(fig, ax, 0.82, 0.41, labels.get('00110', ''), fontsize=number_size)
    draw_text(fig, ax, 0.76, 0.55, labels.get('00111', ''), fontsize=number_size)
    draw_text(fig, ax, 0.51, 0.77, labels.get('01000', ''), fontsize=number_size, color=number_color)
    draw_text(fig, ax, 0.40, 0.20, labels.get('01001', ''), fontsize=number_size)
    draw_text(fig, ax, 0.42, 0.72, labels.get('01010', ''), fontsize=number_size)
    draw_text(fig, ax, 0.50, 0.19, labels.get('01011', ''), fontsize=number_size)
    draw_text(fig, ax, 0.60, 0.74, labels.get('01100', ''), fontsize=number_size)
    #draw_text(fig, ax, 0.62, 0.725, labels.get('01101', ''), fontsize=number_size)
    ax.annotate(labels.get('01101', ''),
                xy=(0.63, 0.72), xycoords='data',fontweight="bold",
                xytext=(90, 60), textcoords='offset points', fontsize=number_size*1.7,
                arrowprops=dict(arrowstyle="->",
                                connectionstyle="angle,angleA=0,angleB=60,rad=10",color="black"))
    draw_text(fig, ax, 0.485, 0.715, labels.get('01110', ''), fontsize=number_size)
    draw_text(fig, ax, 0.62, 0.67, labels.get('01111', ''), fontsize=number_size)
    draw_text(fig, ax, 0.24, 0.59, labels.get('10000', ''), fontsize=number_size, color=number_color)
    draw_text(fig, ax, 0.25, 0.38, labels.get('10001', ''), fontsize=number_size)
    draw_text(fig, ax, 0.75, 0.28, labels.get('10010', ''), fontsize=number_size)
    draw_text(fig, ax, 0.65, 0.25, labels.get('10011', ''), fontsize=number_size)
    draw_text(fig, ax, 0.26, 0.54, labels.get('10100', ''), fontsize=number_size)
    draw_text(fig, ax, 0.265, 0.44, labels.get('10101', ''), fontsize=number_size)
    draw_text(fig, ax, 0.79, 0.38, labels.get('10110', ''), fontsize=number_size)
    draw_text(fig, ax, 0.73, 0.40, labels.get('10111', ''), fontsize=number_size)
    #draw_text(fig, ax, 0.33, 0.66, labels.get('11000', ''), fontsize=number_size, color=number_color)
    ax.annotate(labels.get('11000', ''),
                xy=(0.335, 0.66), xycoords='data',fontweight="bold",
                xytext=(-45, 30), textcoords='offset points', fontsize=number_size*1.7,
                arrowprops=dict(arrowstyle="->",
                                connectionstyle="angle,angleA=0,angleB=120,rad=10",color="black"))
    draw_text(fig, ax, 0.34, 0.30, labels.get('11001', ''), fontsize=number_size)
    #draw_text(fig, ax, 0.35, 0.67, labels.get('11010', ''), fontsize=number_size)
    ax.annotate(labels.get('11010', ''),
                xy=(0.35, 0.667), xycoords='data',fontweight="bold",
                xytext=(-30, 70), textcoords='offset points', fontsize=number_size*1.7,
                arrowprops=dict(arrowstyle="->",
                                connectionstyle="angle,angleA=0,angleB=90,rad=10",color="black"))
    draw_text(fig, ax, 0.51, 0.25, labels.get('11011', ''), fontsize=number_size)
    draw_text(fig, ax, 0.294, 0.56, labels.get('11100', ''), fontsize=number_size)
    draw_text(fig, ax, 0.295, 0.42, labels.get('11101', ''), fontsize=number_size)
    draw_text(fig, ax, 0.37, 0.64, labels.get('11110', ''), fontsize=number_size)
    draw_text(fig, ax, 0.51, 0.47, labels.get('11111', ''), fontsize=number_size, color="black")
    #'''
    # legend
    leg = ax.legend(names, loc='upper left', bbox_to_anchor=(0, 0.9), fancybox=True, fontsize=17, edgecolor="black")
    names  = [name[:-6] for name in names]
    draw_text(fig, ax, 0.15, 0.60, names[0], colors[0], fontsize=text_size, ha="right")
    draw_text(fig, ax, 0.50, 0.84, names[1], colors[1], fontsize=text_size, va="bottom")
    draw_text(fig, ax, 0.87, 0.70, names[2], colors[2], fontsize=text_size, ha="left")
    draw_text(fig, ax, 0.80, 0.15, names[3], colors[3], fontsize=text_size, ha="left")
    draw_text(fig, ax, 0.27, 0.07, names[4], colors[4], fontsize=text_size, ha="right")
    leg.get_frame().set_alpha(0.5)
